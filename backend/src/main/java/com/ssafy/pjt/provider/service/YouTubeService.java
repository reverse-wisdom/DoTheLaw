...
public class YouTubeService {
	...
	public List<YouTubeDTO> get() throws SQLException, IOException {
		...
		try {
			...
			}).setApplicationName("youtube-cmdline-playlist-sample").build();

			PlaylistItemListResponse playlistItemListResponse = youtube.playlistItems().list("snippet").setKey(apiKey)
					.setPlaylistId("UUH5U89kvHrVxxS80xpoOydw") // 한문철 TV
					.setFields("items/snippet").setMaxResults(NUMBER_OF_VIDEOS_RETURNED).execute();

			if (!playlistItemListResponse.getItems().isEmpty()) {
				for (PlaylistItem item : playlistItemListResponse.getItems()) {
					ResourceId rId = item.getSnippet().getResourceId();
